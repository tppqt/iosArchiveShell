...
if [ ${UPLOAD_TYPE} == 1 ]; then
	...
	echo -e "\n"
	if [[ -z $MY_PGY_UK ]]; then
		echo -e "\033[31;40m	error:MY_PGY_UK is null!!     \033[0m"
		exit 0
	fi
	if [[ -z $MY_PGY_API_K ]]; then
		echo -e "\033[31;40m	error:MY_PGY_API_K is null!!     \033[0m"
		exit 0
	fi
	...
	# 校验是否安装fir-cli命令
	if [[ $res2 -eq 0 ]]; then
		echo -e "\033[31;40m	error:not install fir!!!    \033[0m"
		exit 0
	else
		...
		result=$(fir publish ${IPAPATH}/${IPANAME}/${SCHEMENAME}.ipa)
		echo -e "\033[33;40m  \nfir上传结果\n  \033[0m"
		echo -e "  $result  "
	fi
